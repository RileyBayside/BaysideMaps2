...
async function addDataLayers() {
  ...
  map.on('click', (e) => {
    ...
    const html = `<div style="max-width:260px"><h3 style="margin:0 0 6px;font-size:15px;font-weight:600;">Feature Properties</h3><pre style="white-space:pre-wrap;font-size:12px;margin:0;background:#f9fafb;padding:8px;border-radius:8px;border:1px solid #e5e7eb;">${escapeHTML(JSON.stringify(props, null, 2))}</pre></div>`;
...
function fitToData(geojsonData) {
  ...
  const sw = [bbox[0], bbox[1]], ne = [bbox[2], bbox[3]];
